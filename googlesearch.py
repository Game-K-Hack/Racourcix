...
class SearchResult:
    def __init__(self, url, title, description, icon=None, name=None):
        self.url = url
        self.title = title
    ...
    def __repr__(self):
        return f"SearchResult(url={self.url}, title={self.title}, description={self.description}, icon={self.icon}, name={self.name})"
...
def search(term, num_results=10, lang="en", proxy=None, advanced=False, sleep_interval=0, timeout=5, safe="active", ssl_verify=None, region=None):
    ...
    while fetched_results < num_results:
        ...
        for result in result_block:
            ...
            icon = result.find("img")
            if icon is not None:
                icon = icon.get("src")
            name = result.find("cite").parent.parent.find("div").text
            ...
            if link and title and description_box:
                ...
                new_results += 1
                if advanced:
                    yield SearchResult(link["href"], title.text, description, icon, name)
                else:
                    yield link["href"]
